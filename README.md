# Cryptography

В данном репозитории хранятся реализации различных алгоритмов шифрования

## Список

- [Шифр Цезаря](./algorithms/01_Monoalphabetic.ipynb)
- [Шифр перестановки](./algorithms/01_Monoalphabetic.ipynb)
- [Шифр замены](./algorithms/01_Monoalphabetic.ipynb)
- [Шифр Виженера](./algorithms/02_Vigenere.ipynb)
- [Алгоритм Энигмы](./algorithms/03_Enigma.ipynb)
- [DES](./algorithms/04_DES.ipynb)
- [AES](./algorithms/05_AES.ipynb)
- [RSA](./algorithms/06_AES.ipynb)
